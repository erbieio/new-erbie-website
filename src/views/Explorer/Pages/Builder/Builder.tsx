...
import "./Builder.scss";
import { GetStakerOrder, GetValidatorOrder, get_staker_page, get_validator_page, GetValidatorResponse, GetStakerResponse } from '../../../../api/modules/explorer';
...
export default function Validator() {
  ...
  const [validatorData, setValidatorData] = useState<GetValidatorResponse>({data:[], total: 0})
  const [stakerData, setStakerData] = useState<GetStakerResponse>({data:[], total: 0})
      ...
      <div className="flex-1 flex gap-2vh flex-col">
        <div className="flex flex-col lg:flex-row mt-20px lg:mt-0">
          <div className="w-35% flex gap-12px">
            {
              tableMenus.map(item => <div key={item.value} onClick={() => handleFilter(item)} className={`flex-1 data-card ${item.active ? 'active':''}`}>{item.label}</div>)
            }
          </div>
        ...
        <div className="h-64.6vh flex flex-col">
          <div className="table-box h-100%">
            <div className="flex flex-col lg:flex-row justify-between px-10px pt-10px pb-14px">
              ...
              <div>
                <Pagination current={params.current.page} pageSize={params.current.page_size} total={currentFilter === 1 ? validatorData.total : stakerData.total} />
              </div>
            </div>
            <div className="flex h-90%">
              {showTable === 1 ? <ValidatorTable dataSource={validatorData.data} sorter={handleValidatorSorter} /> : <StakerTable dataSource={stakerData.data} sorter={handleStakerSorter} />}
            </div>
          </div>
        </div>
      </div>
